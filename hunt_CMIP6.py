...
12/01/26: Adapted to more easily change criteria parameters
"""
...
today = now.isoformat()
logfilen = 'log_hunt_cmip6_{}.log'.format(today)
logging.basicConfig(filename=logfilen, level=logging.INFO)
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

#Script versio
hunt_vers = "v1.0b 12/01/2026"
...
path_l = '/work/users/clima/lembo/tipping_optimesm/figures_{}'.format(today)
try:
    os.makedirs(path_l)
except OSError:
    pass
...
def data_crunch(f_dir,scen,var,filter,in_year,end_year):
    ...
    if scen=='ssp585' or scen=='ssp245' or scen=='ssp126':
        if var=='siconc' or var=='sos' or var=='tos':
            cdo.setmisstoc(0,
                input= '-selyear,{}/{} -runmean,{} -selvar,{} -setgridtype,,curvilinear -remapcon,r360x180 -yearmean {}'.format(in_year,end_year,filter,var,ofile),
                options = '-P 8',
                output = ofile_my)
            cdo.setmisstoc(0,
                input= '-selyear,{}/{} -selvar,{} -setgridtype,curvilinear -remapcon,r360x180 -yearmean {}'.format(in_year,end_year,var,ofile),
                options = '-P 8',
                output = ofile_y)
            ...
            cdo.runmean(filter,
                input= '-selyear,{}/{} -remapcon,r360x180 -yearmean {}'.format(in_year,end_year,ofile),
                options = '-P 8',
                output = ofile_my)
            cdo.remapcon('r360x180',
        ...
        if var=='siconc' or var=='sos' or var=='tos':
            ...
            cdo.setmisstoc(0,
                input = '-runmean,{} -selvar,{} -setgridtype,curvilinear -remapcon,r360x180 -yearmean {}'.format(filter,var,ofile),
                options = '-P 8',
                output = ofile_my)
            cdo.setmisstoc(0,
                input = '-selvar,{} -setgridtype,curvilinear -remapnn,r360x180 -yearmean {}'.format(var,ofile),
                options = '-P 8',
                output = ofile_y)
        elif var=='zg' or var=='ta' or var=='ua' or var=='va':
            cdo.runmean(filter,
                input= '-remapnn,r360x180 -yearmean -sellevel,{} {}'.format(plev,ofile),
                options = '-P 8',
                output = ofile_my)
            cdo.remapnn('r360x180',
            ...
            # logger.info("Entering CDO manipulations...")
            cdo.runmean(filter,
                input= '-remapnn,r360x180 -yearmean {}'.format(ofile),
                options = '-P 8',
                output = ofile_my)
            cdo.remapnn('r360x180',
